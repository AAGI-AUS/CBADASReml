...
pred_se_avsed_lsd <- function(model, classify, LSD = FALSE, sed = F){
    ...
    prediction_table <- asreml::predict.asreml(model,
                                               classify = classify,
                                               sed = sed)
    ...
    if(sed) avsed <- prediction_table$avsed["mean"] else avsed <- prediction_table$avsed["overall"]
    ...
    av_sed_df <- data.frame(placeholder1 = "Average SED:", placeholder2 = as.character(avsed))
    ...
    if(LSD){
        pred <- invisible(predictPlus.asreml(model, classify, wald.tab = as.data.frame(asreml::wald(model))))

        LSD <- invisible(pred$LSD$meanLSD)

        LSD_df <- data.frame(placeholder1 = "Average LSD:", placeholder2 = as.character(LSD))
    ...
    combined_table_sorted$predicted.value <- as.numeric(combined_table_sorted$predicted.value)
    combined_table_sorted$std.error <- as.numeric(combined_table_sorted$std.error)
